...
<script>
if (jQuery(".portfolio-element").length > 0){
	var text = jQuery("div.portfolio-element ul li:first-child a").text();
jQuery("div.portfolio-element ul li:first-child a").text(text.replace('All', 'Alle'));
}
jQuery('.portfolio-detail-attributes ul li strong').each(function() {
var text = jQuery(this).text();
jQuery(this).text(text.replace('Client', 'Klient').replace('Date', 'Dato').replace('View Project', 'Se mere her'));
});
jQuery('.portfolio-detail-attributes ul li span a').each(function() {
var text = jQuery(this).text();
jQuery(this).text(text.replace('View Project', 'Se mere her'));
});
if (jQuery(".entry-type").length > 0){
	var text = jQuery(".entry-type").text();
jQuery(".entry-type").text(text.replace('Portfolio', 'CASE').replace('Item', ''));
}
jQuery('.portfolio-detail-attributes').addClass("tricky");
</script>
</html>